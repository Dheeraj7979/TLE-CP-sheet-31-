...
int main(){
     ...
     while(t--){
          ...
          while(n--){
               int temp;
               cin>>temp;
               fuelstations.push_back(temp);
          }
          int maxi=fuelstations[0];

          for(int i=0;i<fuelstations.size()-1;i++){
               maxi=max((fuelstations[i+1]-fuelstations[i]),maxi);
          }
          maxi=max(maxi,2*(x-fuelstations[fuelstations.size()-1]));
          cout<<maxi<<endl;
     }
     return 0;
